  ...
  for (int v = mx_len; v >= 0; v--)
    for (int i = 1; i <= n; ++i) {
      for (int j = 1; j <= m; ++j) {
        ...
        double x = 0;
        if (game_map[i][j] == 'S' ) { // prioritize shield
          if (!s_1) x = 4;
        }
        else if (game_map[i][j] != 'D') x = (game_map[i][j] - '0') * 2.5; // Value at current cell is highly prioritized
        if (game_map[i][j] == '0' || game_map[i][j] == 'D') x = -3; // penaltized for going to empty cell

        //x += - 1.0 * dist(x_1, y_1, i, j) * 3 / 4 + 1.0 * dist(x_2, y_2, i, j) / 3; // far from 1 is penaltized or close to 2
        x += -1.0 * dist(x_1, y_1, i, j) / 4; // far from 1 is penaltized
        ...
        if (x >= val[i][j].first) val[i][j] = di(x, (game_map[i][j] == 'D' ? 0 : game_map[i][j] - '0'));
        else val[i][j].second += (game_map[i][j] == 'D' ? 0 : game_map[i][j] - '0');
...
void cook() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (game_map[nx][ny] == '4' || game_map[nx][ny] == '5') // Tat nao choi game 
    ...
    if (equal && best_x == pre_x && best_y == pre_y) larger = true;
    //cout << larger << ' ' << equal << '\n';
